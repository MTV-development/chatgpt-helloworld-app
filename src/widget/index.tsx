...
interface OpenAIWidgetAPI {
  ...
  widgetState?: Record<string, unknown>;  // Property, not method
...
// ============================================================
// POMODORO TIMER - State and rendering
// ============================================================
...
function formatPomodoroTime(ms: number): string {
...
function savePomodoroState(): void {
  if (pomodoroState && window.openai?.setWidgetState) {
    window.openai.setWidgetState(pomodoroState as unknown as Record<string, unknown>);
  }
}
...
function getPomodoroRemaining(): number {
  ...
  if (pomodoroState.completed) return 0;
  if (pomodoroState.isPaused) return pomodoroState.pausedRemaining;
  return Math.max(0, pomodoroState.endTime - Date.now());
}

// Render the circular progress ring
function renderPomodoroRing(remaining: number, total: number, isPaused: boolean, completed: boolean): string {
  const progress = total > 0 ? remaining / total : 0;
  const circumference = 2 * Math.PI * 45; // r=45
  const offset = circumference * (1 - progress);

  let fillClass = "pomodoro-progress-fill";
  if (completed) fillClass += " completed";
...
function renderPomodoro(data: Record<string, unknown>): string {
  // Initialize state from toolOutput or widgetState
  // Note: widgetState is a property, not a method
  const savedState = window.openai?.widgetState as PomodoroState | undefined;
  ...
  if (!pomodoroState) {
    if (savedState && savedState.endTime && !savedState.completed) {
      ...
      pomodoroState = {
        ...
        isPaused: false,
        pausedRemaining: 0,
        label: (data.label as string) || "Focus Session",
        completed: false,
...
function startPomodoroInterval(): void {
  ...
  pomodoroIntervalId = window.setInterval(() => {
    if (!pomodoroState || pomodoroState.isPaused || pomodoroState.completed) return;
...
async function requestPomodoroPiP(): Promise<void> {
  try {
...
(window as unknown as Record<string, unknown>).pausePomodoro = () => {
...
(window as unknown as Record<string, unknown>).resumePomodoro = () => {
  if (pomodoroState && pomodoroState.isPaused && !pomodoroState.completed) {
...
(window as unknown as Record<string, unknown>).resetPomodoro = () => {
  stopPomodoroInterval();
  pomodoroState = null;
...
(window as unknown as Record<string, unknown>).addPomodoroTime = (minutes: number) => {
...
function render(): void {
  ...
  const structuredContent = openai?.toolOutput as Record<string, unknown> | undefined;
