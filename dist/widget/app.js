...
function renderGreeting(data) {
  ...
  return `
    <div class="greeting-display">
      <div class="greeting-emoji">\u{1F44B}</div>
...
function renderTime(data) {
  const formatted = data.formatted || (/* @__PURE__ */ new Date()).toLocaleString();
...
function renderNoData() {
  return `
    <div class="no-data">
      <div class="no-data-icon">\u{1F680}</div>
...
function renderDashboard(data) {
  ...
  const barsHtml = regions.map((region) => {
  ...
  const periodBtns = Object.entries(periodLabels).map(
    ([key, label]) => `
...
function renderRegionDetail(data) {
  ...
  const miniBarsHtml = monthlyData.map((m) => {
